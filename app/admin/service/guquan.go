...
func (this *GuquanList) List() *response.GuquanResp {
	...
	return &response.GuquanResp{
		...
		Price:           float64(m.ReleaseRate) / model.UNITY,
		LimitBuy:        m.LimitBuy,
		LuckyRate:       float64(m.ReleaseRate) / model.UNITY,
		ReturnRate:      float64(m.ReleaseRate) / model.UNITY,
		ReturnLuckyRate: float64(m.ReleaseRate) / model.UNITY,
