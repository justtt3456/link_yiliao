...
func Auth() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		res := global.REDIS.Get(fmt.Sprintf(model.StringKeyAgent, parseToken.Id)).Val()
		u := model.Agent{}
		...
		if err != nil {
			...
			global.REDIS.Set(fmt.Sprintf(model.StringKeyAgent, u.Id), string(bytes), u.ExpireTime())
		}
		if u.Token != parseToken.Key {
			c.JSON(http.StatusOK, gin.H{
				"code": 10000,
